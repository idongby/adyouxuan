 ...
 *    2、获取到 图片的路径 数组
 *    3、把这些图片路径存入到data的一个变量中
 *    4、页面就可以根据 图片数组 进行循环显示 自定义数组
 * 2、点击 自定义图片 组件
 *    1、获取被点击的元素的索引
 *    2、获取data 中的图片数组
 *    3、根据索引 数组中删除对应的元素
 ...
 *    2、对这些内容 合法性验证
 *    3、验证通过 用户选择的图片上传到 专门的图片服务器  返回图片外网的链接
 *      1、遍历图片数组
 *      2、挨个上传
 *      3、自己再维护图片数组 存放 图片上传后的外网的链接
 *    4、文本域 和 外网的图片路径 一起提交到服务器中，前端模拟，不会发送请求到后台
 *    5、清空当前页面
 *    6、返回上一页
 */
Page({
    ...
    if(chooseImage.length !=0){
      chooseImage.forEach((v,i)=>{
        ...
        success: (result) => {
          ...
          console.log(this.UpLoadImge)
          // 所有的图片都上传完毕了，，才触发
          if(i === chooseImage.length-1){
            wx.hideLoading(); //关闭正在上传
            // 上传文本内容和外网的图片数组 提交到后台中
            // 提交都成功,重置页面
            this.setData({
              textVal:"",
              chooseImage:[]
